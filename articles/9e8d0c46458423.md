...
cookieは、Webサーバーが**ユーザーのブラウザに保存する小さなテキストデータ**です。ユーザーがWebサイトを訪問した際に、サーバーからブラウザにHTTPレスポンスヘッダーの`Set-Cookie`として送信され、以降のリクエストではHTTPリクエストヘッダーの`Cookie`として自動的にサーバーに送り返されます。
