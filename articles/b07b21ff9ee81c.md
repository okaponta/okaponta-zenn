---
title: "Zenn問答第13回 「KotlinのValue Classと@JVMInline」"
emoji: "💎"
...
まあでも、型安全性は守られるので、パフォーマンスの恩恵は受けられないものの選択するメリットはありそうです。
...
https://github.com/JetBrains/kotlin/blob/v2.3.0/compiler/frontend/src/org/jetbrains/kotlin/resolve/lazy/descriptors/LazyClassDescriptor.java#L158
にてアノテーションが回収された後、IrClass型が生成され、Inlineであるという情報が保持されます。IRはIntermediate Representationの略で、抽象構文木と機械語の中間の位置付けです。
なので、value classと書くだけでは現在のコンパイラでは不十分でコンパイル時にアノテーションで回収しているというわけですね。

最終的にこちらでunboxされ、バイトコードが生成されるようです。
https://github.com/JetBrains/kotlin/blob/v2.3.0/compiler/ir/backend.jvm/src/org/jetbrains/kotlin/backend/jvm/InlineClassAbi.kt#L35-L49
